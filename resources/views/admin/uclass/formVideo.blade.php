        ...
        <div class="col-lg col-md-12">
          <div class="cover px-4 py-4 rounded-3">
            <div class="headline mb-3">
              <h1 class="fw-bold">Add Materi</h1>
              <hr class="text-warning opacity-100 border-4 rounded" />
            </div>

            <form action="{{ route('uclass.form.kelas.store') }}" enctype="multipart/form-data" method="POST">
            @csrf
              <div class="mb-3">
                ...
                <input type="hidden" value="{{ $kelasInti->id }}" required name="kelas_inti_id" class="form-control shadow-sm" id="exampleFormControlInput1" />
              </div>
              <div class="mb-3">
                <label for="exampleFormControlInput1" class="form-label">Category</label>
                <select class="form-select" disabled  aria-label="select example">                                    
                  <option selected value="{{ $kelasInti->kategori_id }}">{{ $kelasInti->category->title }}</option>                                    
                </select>
                <input type="hidden" value="{{ $kelasInti->kategori_id }}" name="kategori_id">
              </div>
              ...
              <div class="mb-3">
                ...
                <input type="text" name="link_vid" required class="form-control shadow-sm" id="exampleFormControlInput1" title="Please enter a valid YouTube embed video URL">
              </div>
